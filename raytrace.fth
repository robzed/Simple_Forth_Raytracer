...
\ @TODO: Consider converting to full colour
: gcol ( n -- ) 
...
: skyfloor ( -- )  \ Inputs: X,Y,Z,B,U,V,W
                   \ Outputs: B and setting the colour. 
    ...
    v f@ f0< IF \ V<0
        ...
        s>f 2e0 f/ 0.3e0 f+ v f@ fnegate f* 0.2e0 f+ v f!
    then

    \ B=B+3*SQR V
    \ V is used here to be sky or floor gradient, sqrt is gradual shortening 
    V f@ fsqrt 3e0 f* B f@ f+ B f!

    \ Colour is 3-INT(B)
    \ 3-INT B gcol
    3 B f@ f>s - gcol
    \ B=B-INT B ... i.e. get fractional part
    B f@  B f@ ftrunc f- B f!
;
